...
const Users = () => {
  ...
  return (
    <div className={`${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}>
      <div className={`flex flex-col sm:flex-row justify-between ${darkMode ? 'bg-gray-800' : 'bg-yellow-50'} p-5 border border-b-amber-700 font-bold text-xl sticky top-0 z-10`}>
        <h2 className="text-2xl sm:mt-0 text-amber-800">Users</h2>
        <input 
          type="text" 
          id="search" 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-5">
        {filteredCards.length > 0 ? (
          filteredCards.map((card) => (
            <div
              key={card.id}
              className={`flex justify-between font-bold p-4 border rounded-lg shadow-lg cursor-pointer ${card.status === 'active' ? 'border-l-4 border-green-500' : card.status === 'expired' ? 'border-l-4 border-red-500' : 'border-l-4 border-yellow-500'}`}
              
            >
              <img
                src={card.image}
                alt={card.owner}
                className="w-40 h-55 object-cover mb-2"
              />
              <div className="text-start ml-4">
                <h3 className="font-bold text-2xl text-amber-500">{card.owner}</h3>
                <p>Status: {card.status}</p>
                <p>Last Used: {card.lastVisit}</p>
                <p>Goals: {card.goals}</p>
                <p>Coach: {card.coach}</p>
                <button onClick={() => handleCardClick(card)} className= {` py-1 px-4  m-2 border rounded-lg shadow-lg cursor-pointer ${card.status === 'active' ? 'border-l-4 border-green-500' : card.status === 'expired' ? 'border-l-4 border-red-500' : 'border-l-4 border-yellow-500'}`}>More detail</button>
